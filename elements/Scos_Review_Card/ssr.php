 ...
 * v3.0 | 2026-06-20
 *
 * Restored to the working do_shortcode pattern from the pre-connected-mode
 * version (xCustom Breakdance Blocks). Inline rendering approaches (closures,
 * direct echo) all leaked on LiteSpeed PHP 8.0 regardless of ob_start nesting.
 * The do_shortcode → render() → ob_start → ob_get_clean → string → echo chain
 * is the only approach confirmed working on this server.
 *
 * Connected mode is disabled — will be implemented as a separate element.
 *
...
// Support both current path (content.display.layout) and old path (content.layout).
$layout = isset( $content['display']['layout'] )
	? (string) $content['display']['layout']
	: ( isset( $content['layout'] ) ? (string) $content['layout'] : 'stacked' );

// Connected mode not implemented — will be a separate element.
if ( 'connected' === $mode ) {
	if ( defined( 'BREAKDANCE_BUILDER' ) && BREAKDANCE_BUILDER ) {
		echo '<div class="bde-scos-review-card__placeholder">'
			. esc_html__( 'Connected mode is not available — use Specific or Loop mode.', 'site-essentials' )
			. '</div>';
	}
	return;
}
...
// Field toggles — 1/0 strings for the shortcode.
$bool = static function ( $val, bool $default = true ): string {
	if ( $val === null || $val === '' ) {
		return $default ? '1' : '0';
	}
	return $val ? '1' : '0';
...
// Build shortcode attribute string and render.
$att_string = '';
foreach ( $atts as $k => $v ) {
	$att_string .= ' ' . $k . '="' . esc_attr( (string) $v ) . '"';
}

echo do_shortcode( '[bw_review_card' . $att_string . ']' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
